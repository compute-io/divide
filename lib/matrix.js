...
*	Computes an element-wise division of a matrix
*
...
function divide( out, x, y ) {
	...
	if ( isMatrixLike( y ) ) {
		...
		if ( M !== x.shape[0] || N !== y.shape[1] ) {
			throw new Error( 'divide()::invalid input arguments. Divisor matrix must have the same number of rows and columns as the input matrix.' );
		}
		for ( i = 0; i < M; i++ ) {
			for ( j = 0; j < N; j++ ) {
